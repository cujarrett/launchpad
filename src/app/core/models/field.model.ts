...
export function parseSchema(paramsSchema: Record<string, unknown>): FieldDef[] {
  ...
  return Object.entries(properties)
    ...
    .map(([key, schema]) => classifyField(key, schema as Record<string, unknown>, required.includes(key)))
}
...
function classifyField(key: string, schema: Record<string, unknown>, required: boolean): FieldDef {
  ...
  if (schema['type'] === 'object') {
    ...
    return { ...base, kind: 'sub-object', children, ...(hasEnabledChild ? { connection: true } : {}) }
  }
...
const ACRONYMS = new Set(['cpu', 'ram', 'url', 'api', 'id', 'ip', 'db', 'ssl', 'tls', 'iam', 'aws', 'gcp', 'sql', 'nosql'])

...
read -rs NEW_TOKEN
echo
gh secret set HOMELAB_PAT --repo cujarrett/launchpad --body "$NEW_TOKEN"
unset NEW_TOKEN

# 3. Revoke the old token, then confirm the next merge to main still deploys
gh run watch --repo cujarrett/launchpad \
  "$(gh run list --repo cujarrett/launchpad --branch main --limit 1 --json databaseId --jq '.[0].databaseId')"
```
